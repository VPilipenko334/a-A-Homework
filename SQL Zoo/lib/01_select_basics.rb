...
def per_capita_gdp
  # Show the name and per capita gdp 
  #(gdp/population) for each country where
  # the area is over 5,000,000 km^2
  execute(<<-SQL)
    SELECT 
      name, gdp/population AS gdp_population
    FROM 
     countries  
    WHERE 
      area > 5000000;
...
def small_and_wealthy
  # Show the name and continent of countries 
  # where the area is less than 2,000
...
def just_the_right_size
  # Show the country and the area in 1000's 
  # of square kilometers for countries
  # with an area between 200,000 and 250,000.
  # BETWEEN allows range checking - 
  # note that it is inclusive.
  execute(<<-SQL)
  SELECT 
    name, area/1000
  FROM 
